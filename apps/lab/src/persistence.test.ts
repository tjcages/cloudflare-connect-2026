...
describe("config file import/export", () => {
  ...
  it("uses the Cloudflare marketing palette and Connect shader as factory defaults", () => {
    expect(DEFAULT_LAB_ENGINE_CONFIG.stripeDots).toEqual({
      // Disabled in 1bdb17a so the white underlay reads behind the Twizzler ribbon (#CF-15).
      enabled: false,
      density: 0.8,
      randomVisibility: 1,
      sizePx: 1.5,
    ...
    expect(DEFAULT_LAB_ENGINE_CONFIG.stripeBorder).toEqual({
      // Disabled in 1bdb17a alongside stripeDots for the Twizzler underlay (#CF-15).
      enabled: false,
      minWidthPx: 4,
      density: 0.02,
    });
