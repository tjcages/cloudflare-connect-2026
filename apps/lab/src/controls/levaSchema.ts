...
export function useEngineControls(
        ...
        Presets: drawerFolder(
          "Presets",
          {
            ...
            clientCanvasWidth: {
              ...
              min: 1,
              max: MAX_CLIENT_CANVAS_DIMENSION_PX,
              step: 1,
              label: "Width (px)",
              render: (get) =>
  ...
  const clientSizeUnit = String(
    (shaderValues as unknown as Record<string, unknown>).clientSizeUnit ??
      initialLabSettings.clientSizeUnit ??
      "pixels",
  ) as ClientSizeUnit;
  const clientSizePpi = normalizeClientPrintPpi(
    Number(
      (shaderValues as unknown as Record<string, unknown>).clientSizePpi ??
        initialLabSettings.clientSizePpi ??
        DEFAULT_CLIENT_PRINT_PPI,
    ),
  );
  const clientCanvasWidthInches = Number(
    (shaderValues as unknown as Record<string, unknown>).clientCanvasWidthInches ??
      initialLabSettings.canvasWidth / clientSizePpi,
  );
  const clientCanvasHeightInches = Number(
  ...
  useEffect(() => {
    ...
    if (clientSizeUnit === "inches") {
      setShaderControl({
        ...
        clientCanvasHeightInches: Number((previousPixels.height / clientSizePpi).toFixed(3)),
      });
    } else {
      setShaderControl({
        clientCanvasWidth: previousPixels.width,
        clientCanvasHeight: previousPixels.height,
      });
    }
  ...
  return {
    ...
    twizzler: {
      ...
      ...normalizeTwizzlerSettings({
        ...
        // CF-80: leftover sine/parametric knobs are not in Leva; keep stored values.
        leftHeight: initialLabSettings.twizzler.leftHeight,
        rightHeight: initialLabSettings.twizzler.rightHeight,
        edgeFluctuation: initialLabSettings.twizzler.edgeFluctuation,
        edgeSpeed: initialLabSettings.twizzler.edgeSpeed,
        edgeTaper: initialLabSettings.twizzler.edgeTaper,
        wrinkles: initialLabSettings.twizzler.wrinkles,
        wrinkleStrength: initialLabSettings.twizzler.wrinkleStrength,
        bendPosition: initialLabSettings.twizzler.bendPosition,
        bendAmount: initialLabSettings.twizzler.bendAmount,
        bend2Position: initialLabSettings.twizzler.bend2Position,
        bend2Amount: initialLabSettings.twizzler.bend2Amount,
        bend3Position: initialLabSettings.twizzler.bend3Position,
        bend3Amount: initialLabSettings.twizzler.bend3Amount,
        depthPosition: initialLabSettings.twizzler.depthPosition,
        depthAmount: initialLabSettings.twizzler.depthAmount,
        depthWidth: initialLabSettings.twizzler.depthWidth,
        ...
        noiseScaleX: initialLabSettings.twizzler.noiseScaleX,
        noiseScaleY: initialLabSettings.twizzler.noiseScaleY,
        speed: shaderValueRecord.twizzlerSpeed,
        drift: initialLabSettings.twizzler.drift,
        stippleSize: initialLabSettings.twizzler.stippleSize,
        stippleGap: initialLabSettings.twizzler.stippleGap,
        rotateX: initialLabSettings.twizzler.rotateX,
        rotateY: initialLabSettings.twizzler.rotateY,
        rotateZ: initialLabSettings.twizzler.rotateZ,
        panX: shaderValueRecord.twizzlerPanX,
        panY: shaderValueRecord.twizzlerPanY,
        panZ: shaderValueRecord.twizzlerPanZ,
        viewDistance: initialLabSettings.twizzler.viewDistance,
      }),
    },
