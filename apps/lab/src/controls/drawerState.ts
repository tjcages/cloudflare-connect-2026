...
function readControlDrawerState(): ControlDrawerState {
  ...
  try {
    ...
    return parsed && typeof parsed === "object" ? (parsed as ControlDrawerState) : {};
  } catch {
    return {};
  }
}
...
export function loadControlDrawerOpen(id: ControlDrawerId, defaultOpen = false): boolean {
  const state = readControlDrawerState();
  return typeof state[id] === "boolean" ? state[id] : defaultOpen;
}

export function saveControlDrawerOpen(id: ControlDrawerId, open: boolean): void {
  const state = readControlDrawerState();
  state[id] = open;
...
export function saveControlDrawerSnapshot(snapshot: Record<string, unknown> | undefined): void {
  if (!snapshot) return;
