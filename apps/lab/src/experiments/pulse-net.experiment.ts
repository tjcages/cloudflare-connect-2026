...
import { PULSE_NET_FRAG, PULSE_NET_MAX_LINES, PULSE_NET_MAX_PULSES, PULSE_NET_STAR_COUNT } from "./pulse-net.shaders";
...
function buildStars(): StarSet {
  const n = PULSE_NET_STAR_COUNT;
  const rng = createSeededRng(3407);
...
function buildGraph(stars: StarSet, cssW: number, cssH: number, linkRadius: number): Graph {
  ...
  const byDegree = Array.from({ length: n }, (_, i) => i).sort((p, q) => incident[q].length - incident[p].length);
  const hubs: number[] = [];
  ...
  return { sx, sy, edges, incident, waypoints };
}
...
function launchPulse(state: SimState, graph: Graph, edgeIndex: number, from: number, now: number, hops: number): void {
  if (state.pulses.length >= PULSE_NET_MAX_PULSES) return;
  const edge = graph.edges[edgeIndex];
  if (now - edge.lastPulse < RELAY_COOLDOWN_MS) return;
  edge.lastPulse = now;
  const span = edgeLength(graph, edge) / Math.max(1, state.linkRadius * PAIR_DIST_SCALE);
...
function relaxStarActivation(state: SimState, dt: number): void {
  ...
  for (let i = 0; i < PULSE_NET_STAR_COUNT; i++) {
...
function updateSim(state: SimState, frame: FieldHookFrame): void {
  ...
  let count = 0;
  for (const edge of graph.edges) {
    if (count >= PULSE_NET_MAX_LINES) break;
  ...
  for (const pulse of state.pulses) {
    if (pulseCount >= PULSE_NET_MAX_PULSES) break;
    const edge = graph.edges[pulse.edge];
...
const definition: ExperimentDefinition = {
  ...
  category: "stars",
  blurb:
    "Cursor-linked constellation lines carry travelling light packets; arriving pulses flare hub stars and relay onward.",
  create: (ctx) => {
    const state: SimState = {
      ...
      pulses: [],
      pending: [],
      flareStart: new Float32Array(PULSE_NET_STAR_COUNT).fill(-1e9),
      segData: new Float32Array(PULSE_NET_MAX_LINES * 4),
      alphaData: new Float32Array(PULSE_NET_MAX_LINES),
      pulseSegData: new Float32Array(PULSE_NET_MAX_PULSES * 4),
      pulseFxData: new Float32Array(PULSE_NET_MAX_PULSES * 2),
      lineCount: 0,
      pulseCount: 0,
      starAct: new Float32Array(PULSE_NET_STAR_COUNT),
      starFlare: new Float32Array(PULSE_NET_STAR_COUNT),
      starPeak: new Float32Array(PULSE_NET_STAR_COUNT),
      starSum: new Float32Array(PULSE_NET_STAR_COUNT),
    };

    const fieldPass = ({ gl, quad }: EngineHookContext): FieldHookPass => {
      const program = compileProgram(gl, FULLSCREEN_VERT, PULSE_NET_FRAG);
      const starTexture = createDataTexture(gl, state.stars.bytes, PULSE_NET_STAR_COUNT, 1);
      ...
      return {
        render(frame) {
          ...
          gl.uniform1fv(uLineAlpha, state.alphaData);
