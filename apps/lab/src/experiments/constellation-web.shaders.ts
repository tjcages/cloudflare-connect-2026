...
export const CONSTELLATION_MAX_PULSES = 20;
...
uniform vec4 uPulseSeg[${CONSTELLATION_MAX_PULSES}];
uniform vec2 uPulseFx[${CONSTELLATION_MAX_PULSES}];
uniform float uStarAct[${CONSTELLATION_STAR_COUNT}];
uniform float uStarFlare[${CONSTELLATION_STAR_COUNT}];
...
void main() {
  ...
  for (int i = 0; i < ${CONSTELLATION_MAX_PULSES}; i++) {
  ...
  for (int i = 0; i < ${CONSTELLATION_STAR_COUNT}; i++) {
    ...
    float rc = rcBase * (1.0 + 1.35 * grow + 0.85 * flare);
