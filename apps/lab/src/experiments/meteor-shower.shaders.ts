...
void meteorContribution(
  ...
  float channelWidth = (7.0 + 13.0 * taper) * bulk;
  float amount = pushProfile(axisDistance, channelWidth) * channelWidth * 0.62 * envelope * taper;
  vec2 pushDirection = axisDistance > 0.0001 ? offset / axisDistance : vec2(-direction.y, direction.x);
  warp -= pushDirection * amount;

  float drag = exp(-axisDistance / max(1.0, channelWidth * 1.4)) * envelope * taper * 17.0 * bulk;
  warp -= direction * drag;
  ...
  float bow = pushProfile(headDistance, headRadius * 2.0) * headRadius * 1.15 * envelope;
