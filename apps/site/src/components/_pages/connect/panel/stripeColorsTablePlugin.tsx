...
type StripeColorsTablePluginProps = LevaInputProps<
  string,
  StripeColorsTableSettings
>;

export function stripeSyncKey(stripes: readonly EditableStripe[]): string {
  return stripes
    .map((s) => `${s.hex}:${s.startFrom}:${s.width}:${s.opacity}`)
    .join("|");
}
    ...
    <Row>
      <div className="playground-stripe-colors-leva-row w-full min-w-0">
        <StripeColorsTable
          ...
          thresholdEasingOptions={
            stripeColorsTableRuntime.thresholdEasingOptions
          }
...
export const stripeColorsTablePlugin = createPlugin<
  StripeColorsTableInput,
  string,
  StripeColorsTableSettings
>({
