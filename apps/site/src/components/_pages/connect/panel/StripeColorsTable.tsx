...
function LevaRangeSlider({
  ...
  const bind = useDrag(
    ({ event, first, xy: [x], movement: [mx], memo }: any) => {
      if (first) {
        ...
        memo = targetIsScrub
          ? value
          : invertedRange((x - bounds.left) / bounds.width, min, max);
      }
...
function LevaSliderControl({
  ...
  return (
    <div
      className="hasRange stripe-colors-leva-range-grid"
      aria-label={ariaLabel}
    >
      <LevaRangeSlider
        value={value}
    ...
    <div className="stripe-colors-easing-row">
      ...
      <div className="stripe-colors-easing-control">
        <LevaSelectControl
          ...
          onChange={(nextValue) =>
            onChange(nextValue === "__custom" ? customValue : nextValue)
          }
        />
    ...
    >
      <div className="stripe-colors-color-header">
        <button
          ...
          disabled={disabled}
          onPointerDown={
            disabled ? undefined : (event) => controls.start(event)
          }
        >
        ...
        {showColorControls ? (
          <>
            <HexColorPopover
              ...
              triggerStyle={
                {
                  "--stripe-swatch-color": cssColorForHex(stripe.hex),
                } as CSSProperties
              }
              align="right"
            />
            <div className="stripe-colors-color-meta">
              ...
              <span className="stripe-colors-color-code">
                [{colorMeta.code}]
              </span>
            </div>
          </>
          ...
          <>
            <div
              className="stripe-colors-leva-swatch stripe-colors-leva-swatch-placeholder"
              aria-hidden="true"
            />
            <div className="stripe-colors-color-meta">
              <span className="stripe-colors-color-name">
                Level {index + 1}
              </span>
...
export function StripeColorsTable({
  ...
  const reconcileKey = stripes
    .map((s) => `${s.id}:${s.hex}:${s.opacity}:${s.startFrom}:${s.width}`)
    .join("|");
  ...
  const hasEasingControls =
    ...
    (Object.keys(thresholdEasingOptions).length > 0 &&
      !!onThresholdEasingChange);
  const hasPaletteActions =
    !!onShufflePalette || !!onUndoShuffle || !!onReverseColorOrder;
