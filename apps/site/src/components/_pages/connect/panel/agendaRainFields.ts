...
import { BLEND_MODES, RENDER_MODES } from "./speakerFramesFields";

/**
 * The agenda-rain surface of the lab's Leva panel, translated 1:1 into
 * @tjcages/panels field defs: the Rain stream knobs, texture source framing,
 * grid geometry, the stripe palette table, tone and background FX — the lab's
 * Graphic Rain controls bound to the site's `AgendaRainSettings`.
 */

/** Panel values: the settings record with stripes in the table's row shape. */
export function seedAgendaRainPanelValues(
  settings: AgendaRainSettings
): PanelValues {
  return { ...settings, stripes: toEditableControls(settings.stripes) };
}

export function agendaRainFromPanelValues(
  values: PanelValues
): AgendaRainSettings {
  return {
    ...(values as unknown as AgendaRainSettings),
    stripes: fromEditableControls(values.stripes as EditableStripe[]),
  };
}

export function buildAgendaRainSections(): PanelSectionDef[] {
  return [
    {
      id: "Rain",
      title: "Rain",
    ...
    },
    {
      ...
      defaultOpen: true,
      fields: [
        select("shaderPreset", "Shader", rainShaderOptions()),
        num("sourceSpeed", "Source speed", 0, 4, 0.01),
        num("sourceScale", "Source resolution", 0.25, 2, 0.05),
        num("sourceZoom", "Source zoom", 0.2, 5, 0.01),
        num("sourcePanX", "Source pan X", -1, 1, 0.01),
        num("sourcePanY", "Source pan Y", -1, 1, 0.01),
        num("sourceRotateX", "Rotate X", -89, 89, 1),
        num("sourceRotateY", "Rotate Y", -180, 180, 1),
        num("sourceRotateZ", "Rotate Z", -180, 180, 1),
      ],
    },
    {
      id: "Grid geometry",
    ...
    },
    {
      ...
      title: "Background",
      fields: [
        toggle("backgroundTransparent", "Transparent"),
    ...
    },
    {
      ...
      fields: [
        ...
        num("renderIntensity", "Render intensity", 0, 1, 0.01),
