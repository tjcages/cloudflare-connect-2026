...
import {
  GradientStopsEditor,
  type GradientStopsEditorLayout,
...
export function setTwizzlerGradientEditorLayout(
...
function subscribeTwizzlerGradientEditorLayout(
  listener: () => void
): () => void {
...
function GradientStopsPluginComponent() {
  const { value, onUpdate, disabled, settings } =
    useInputContext<GradientStopsPluginProps>();
  const layout = useSyncExternalStore(
    subscribeTwizzlerGradientEditorLayout,
    () => editorLayout,
    () => editorLayout
  );
  const stops = parseTwizzlerGradientStops(
    value,
    settings.colorFar,
    settings.colorNear
  );
    ...
    <Row>
      <div className="twizzler-gradient-leva-row w-full min-w-0">
        <GradientStopsEditor
          ...
          stops={stops}
          disabled={disabled}
          onChange={(next: TwizzlerGradientStop[]) =>
            onUpdate(serializeTwizzlerGradientStops(next))
          }
        />
      </div>
    </Row>
...
export const gradientStopsPlugin = createPlugin<
  GradientStopsInput,
  string,
  GradientStopsSettings
>({
  component: GradientStopsPluginComponent,
  normalize: (input) => {
    const record = input && typeof input === "object" ? input : null;
    const colorFar =
      record && typeof record.colorFar === "string"
        ? record.colorFar
        : "#fea700";
    const colorNear =
      record && typeof record.colorNear === "string"
        ? record.colorNear
        : "#f46021";
    const raw = record && "value" in record ? record.value : input;
    return {
      value: serializeTwizzlerGradientStops(
        parseTwizzlerGradientStops(raw, colorFar, colorNear)
      ),
  ...
  sanitize: (value) =>
    serializeTwizzlerGradientStops(
      parseTwizzlerGradientStops(value, "#fea700", "#f46021")
    ),
});
