...
export type ClickWaveSample = ClickWavePoint & {
  ...
  waveProgress: number;
  /** Unitless radial push multiplier, strongest at birth (strength applied at rasterize time). */
  pushPower: number;
  /** Unitless ring brighten multiplier; fades slower than push (may exceed 1 at birth). */
  whitePower: number;
  /** Deterministic per-wave seed for the rasterizer's wobble/breakup/dissolve noise. */
  seed: number;
};
...
/** Explosive expansion: most of the radius is covered in the first third of life. */
