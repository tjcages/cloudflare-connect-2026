import type { TranscodeWebmToMp4Options } from "./playgroundFfmpeg";
import { createPlaygroundExportCompositor, type PlaygroundVideoBackgroundOptions } from "./playgroundVideoCompositor";

export const PLAYGROUND_IMAGE_EXPORT_DURATION_SEC = 5;
export const PLAYGROUND_VIDEO_EXPORT_MAX_DURATION_SEC = 120;
export const PLAYGROUND_VIDEO_EXPORT_CONFIRM_THRESHOLD_SEC = 60;
/** 24fps keeps stripe motion smooth while cutting frame count vs 30fps. */
export const PLAYGROUND_VIDEO_EXPORT_FPS = 24;
/** Caps encoder work and file size for 2x retina graphic output. */
export const PLAYGROUND_VIDEO_EXPORT_BITRATE = 6_000_000;
...
export function formatVideoExportStatusLabel(
  phase: PlaygroundVideoExportPhase,
...
export function resolveExportDuration(kind: PlaygroundExportSourceKind, videoDuration: number): number {
  ...
  return Math.min(videoDuration, PLAYGROUND_VIDEO_EXPORT_MAX_DURATION_SEC);
}

export function resolveExportFrameCount(durationSec: number): number {
  return Math.max(1, Math.ceil(durationSec * PLAYGROUND_VIDEO_EXPORT_FPS));
}

export function shouldConfirmLongExport(durationSec: number): boolean {
  return durationSec > PLAYGROUND_VIDEO_EXPORT_CONFIRM_THRESHOLD_SEC;
}
...
export function canUseFastVideoExport(canvas: HTMLCanvasElement, sourceKind: PlaygroundExportSourceKind): boolean {
...
export function downloadPlaygroundVideo(blob: Blob, filename = "ascii-video.mp4"): void {
...
/** @deprecated Use {@link downloadPlaygroundVideo}. */
export const downloadPlaygroundMp4 = downloadPlaygroundVideo;
...
function restoreVideoState(video: HTMLVideoElement, saved: SavedVideoState): void {
  ...
  if (!saved.paused) {
    void video.play().catch(() => {
      /* ignore autoplay rejection after export */
    });
  } else {
    video.pause();
  }
...
export type ExportPlaygroundVideoOptions = {
  canvas: HTMLCanvasElement;
  sourceKind: PlaygroundExportSourceKind;
  video?: HTMLVideoElement;
  backgroundCss?: string;
  backgroundColor?: number;
  signal?: AbortSignal;
  onPhase?: (phase: PlaygroundVideoExportPhase) => void;
  onProgress?: (elapsedMs: number, totalMs: number) => void;
...
export async function exportPlaygroundVideo(options: ExportPlaygroundVideoOptions): Promise<Blob> {
  const {
    canvas,
    sourceKind,
    video,
    ...
    filename,
    transcode,
    createCompositor = createPlaygroundExportCompositor,
  } = options;
  ...
  const outputFilename = filename ?? `ascii-video.${profile.extension}`;
  ...
  const compositor = await createCompositor(canvas, { backgroundCss, backgroundColor });
  ...
  try {
    const recorderOptions = {
      fps: PLAYGROUND_VIDEO_EXPORT_FPS,
      durationMs,
      mimeType: profile.mimeType,
      videoBitsPerSecond: PLAYGROUND_VIDEO_EXPORT_BITRATE,
      signal,
      onProgress,
      compositeFrame,
    };
  ...
  if (!profile.skipTranscode) {
    const transcodeFn = transcode ?? (await import("./playgroundFfmpeg")).transcodeWebmToMp4;
  ...
  if (download) {
    downloadPlaygroundVideo(outputBlob, outputFilename);
  }
