...
- `apps/lab/` — the authoring app and the **live Cloudflare worker** (`section-grid-generator`, served at `/`).
...
- `pir deploy` builds the lab and runs `wrangler deploy` (the `section-grid-generator` worker).
