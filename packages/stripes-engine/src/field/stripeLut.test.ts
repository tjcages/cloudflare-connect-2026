...
describe("buildStripeOpacityLut", () => {
  ...
  it("packs opacity into r/b, ramp position into g, alpha 255", () => {
    ...
    const i = 255 * 4;
    expect([lut[i], lut[i + 1], lut[i + 2], lut[i + 3]]).toEqual([128, 255, 128, 255]);
  });
  it("bands below the lowest threshold pack zero opacity with alpha 255", () => {
    ...
    expect([lut[mid], lut[mid + 1], lut[mid + 2], lut[mid + 3]]).toEqual([64, 0, 64, 255]);
  });
});
