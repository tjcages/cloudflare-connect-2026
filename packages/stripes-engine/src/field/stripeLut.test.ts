...
import { buildStripeLut, buildStripeOpacityLut, lutSignature, WIDTH_LUT_SCALE } from "./stripeLut";
...
describe("buildStripeOpacityLut", () => {
  ...
  it("packs opacity into rgb with alpha 255", () => {
    const lut = buildStripeOpacityLut(STRIPES);
    expect([lut[0], lut[1], lut[2], lut[3]]).toEqual([255, 255, 255, 255]);
    const i = 255 * 4;
    expect([lut[i], lut[i + 1], lut[i + 2], lut[i + 3]]).toEqual([128, 128, 128, 255]);
  });
  it("bands below the lowest threshold pack zero opacity with alpha 255", () => {
    ...
    const mid = 128 * 4; // t≈0.502 → band 0 (opacity 0.25 → 64)
    expect([lut[mid], lut[mid + 1], lut[mid + 2], lut[mid + 3]]).toEqual([64, 64, 64, 255]);
  });
});
