export function buildConstellationFrag(maxStars: number, maxLinks: number, maxPulses: number): string {
...
uniform vec4 uLineSeg[${maxLinks}];
uniform vec2 uLineFx[${maxLinks}];
uniform vec4 uPulseSeg[${maxPulses}];
uniform vec2 uPulseFx[${maxPulses}];
