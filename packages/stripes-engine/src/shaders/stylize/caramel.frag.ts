...
void main(){
  float t = uTime * 0.3;
  float dripAmt = mix(0.0, 0.05, uParams.x);
  float glossAmt = uParams.y;
  float depth = uParams.z;
  vec2 uv = vUv;
  uv.y += fbm(vec2(vUv.x * 30.0, vUv.y * 4.0) + t) * dripAmt * uIntensity;
  uv = warp(uv, vec2(40.0, 12.0), 0.012 * uIntensity, t);
  vec3 c = texture(uTex, uv).rgb;
  float l = luma(c);
  c = mix(c, vec3(0.8, 0.4, 0.08) * (0.4 + l), mix(0.4, 0.9, depth) * uIntensity);
  vec3 hi = blurTex(uv - vec2(2.0, 3.0) / uResolution, 3.0 * uDpr);
  float sheen = smoothstep(0.45, 0.95, luma(hi));
  c += sheen * glossAmt * 0.6 * uIntensity;
