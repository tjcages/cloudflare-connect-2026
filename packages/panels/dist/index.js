...
var clamp = (v, min, max) => Math.min(Math.max(v, min), Math.max(min, max));
...
function EmbeddedPicker({
  ...
  const updateSaturation = useCallback(
    (clientX, clientY) => {
      ...
      if (!rect) return;
      const s = clamp2((clientX - rect.left) / Math.max(1, rect.width) * 100, 0, 100);
      const v = clamp2(
  ...
  const updateHue = useCallback(
    (clientX) => {
      ...
      if (!rect) return;
      const h = clamp2((clientX - rect.left) / Math.max(1, rect.width) * 360, 0, 360);
