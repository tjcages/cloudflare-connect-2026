...
import { type PlaygroundRevealConfig } from "./playgroundRevealConfig";
...
// ON or OFF and in the Field debug stage. Handles WAVE only; assembly reveal is handled by the
// GPU mesh in assemblyPuzzlePass.ts (pre-stripe field displacement). When assembly is active the
// filter is not in the chain, so it only needs to be pass-through for assembly. WebGL1-safe (R6).
