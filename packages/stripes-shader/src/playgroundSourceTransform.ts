...
export function isDefaultPlaygroundSourceTransform(input: PlaygroundSourceTransform): boolean {
  const base = DEFAULT_PLAYGROUND_SOURCE_TRANSFORM;
  return input.fit === base.fit && input.zoom === base.zoom && input.panX === base.panX && input.panY === base.panY;
}
