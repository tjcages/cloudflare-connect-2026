...
import {
  ...
  ICON_BOX_1X2_CENTER_STROKE_TRAVEL_PX,
...
export const buildIconBox1x2CenterStroke = (
  themeFillRgb: number,
): { strokeRoot: Container; disposeCenterStrokeAnim: () => void } => {
  const anchor = getIconBox1x2CenterStrokeAnchor();
  ...
  gfx.rect(-halfW, -halfH, ICON_BOX_1X2_CENTER_STROKE_WIDTH, ICON_BOX_1X2_CENTER_STROKE_HEIGHT).fill({
    color: themeFillRgb,
  });
  strokeRoot.addChild(gfx);

  const disposeCenterStrokeAnim = startIconBox1x2CenterStrokeLoop(strokeRoot, anchor.y);
 ...
 */
export const startIconBox1x2CenterStrokeLoop = (strokeRoot: Container, anchorY: number): (() => void) => {
  const travel = ICON_BOX_1X2_CENTER_STROKE_TRAVEL_PX;
  const offsetY = motionValue(0);

  const unsub = offsetY.on("change", (dy) => {
    strokeRoot.position.y = anchorY + dy;
  });
  ...
  const runLoop = async () => {
    ...
    while (!cancelled) {
      ...
      activeControls = animate(offsetY, nextOffset, {
        duration,
