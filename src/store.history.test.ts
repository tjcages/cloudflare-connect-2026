...
import { resetGridGenerationSchedulerForTests } from "./store/gridGeneration";
...
describe("document history", () => {
  ...
  it("undoes and redoes grid config changes while keeping the generated grid in sync", async () => {
    vi.useFakeTimers();
    ...
    expect(useAppStore.getState().grid).toEqual(
      generateGrid({ ...useAppStore.getState().gridConfig, strokeColor: "#ff0000" }),
    );
    ...
    expect(useAppStore.getState().grid).toEqual(generateGrid(useAppStore.getState().gridConfig));
