/** Sparkle gap mask for stripe grid cells. GLSL in stripeFilterShaders must stay aligned. */
...
export function resolvePersistedSparkleGapsActivePercent(config: {
  ...
  /** @deprecated Migrated from sparkleRate / sparkleEnabled. */
  sparkleRate?: number;
...
export function resolvePersistedSparkleGapsSpeed(config: {
  ...
  /** @deprecated Migrated from sparkleRate (slider 0–1). */
  sparkleRate?: number;
  sparkleEnabled?: boolean;
}): number {
...
/** Deterministic 0..1 hash per grid cell (legacy; kept for tests). */
...
/** @deprecated Use {@link playgroundSparkleOptionsFromSliders}. */
