/**
 * Capture the CF-16 macro-hill A/B/C axis with every non-axis setting locked.
 *
 * Usage:
 *   node scripts/capture-twizzler-macro-hills.mjs
 */
...
if (base.lineCount !== 240) {
  throw new Error(`CF-16 macro-hill capture requires the 240-line lock; received ${base.lineCount}`);
}

const variants = [
  { id: "A", label: "calmer · fewer broad hills", hillRhythm: 0 },
  { id: "B", label: "current locked energy", hillRhythm: 1 },
...
for (const variant of variants) {
  ...
  const path = resolve(outDir, `twizzler-macro-hills-${variant.id}.png`);
...
const stackPath = resolve(outDir, "twizzler-macro-hills-STACK.png");
...
writeFileSync(resolve(outDir, "twizzler-macro-hills-manifest.json"), JSON.stringify(manifest, null, 2));
