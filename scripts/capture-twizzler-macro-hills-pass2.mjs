/**
 * Capture CF-16 macro-hill pass 2 without overwriting pass-1 artifacts.
 *
 * Usage:
 *   node scripts/capture-twizzler-macro-hills-pass2.mjs
 */
...
const bundlePath = "/tmp/twizzler-macro-hills-pass2-bundle.js";
...
if (base.lineCount !== 240) {
  throw new Error(`CF-16 macro-hill pass 2 requires the 240-line lock; received ${base.lineCount}`);
}

const variants = [
  { id: "A2", label: "long calm opening → lifted terminal wave", hillRhythm: 0 },
  { id: "B2", label: "strong/soft cadence → pronounced smooth lift", hillRhythm: 1 },
  { id: "C2", label: "asymmetric multi-hills → right-edge acceleration", hillRhythm: 2 },
];
...
for (const variant of variants) {
  ...
  const path = resolve(outDir, `twizzler-macro-hills-pass2-${variant.id}.png`);
...
const stackPath = resolve(outDir, "twizzler-macro-hills-pass2-STACK.png");
...
const manifest = {
  pass: 2,
  synthesis: ["S1 gentler bends", "S2 stronger bends", "S3 more depth lift"],
  locked: {
...
writeFileSync(resolve(outDir, "twizzler-macro-hills-pass2-manifest.json"), JSON.stringify(manifest, null, 2));
