/**
 * CF-16 pixel-drift pass 3: adaptive contours and denser ridge reconstruction.
 *
 * Raster pixels are transient evidence only. Exported SVGs contain asserted
 * path geometry with no embedded raster content.
 */
...
const traceBundlePath = "/tmp/cf-16-raster-drift-pass3-trace-bundle.js";
const twizzlerBundlePath = "/tmp/cf-16-twizzler-pass3-render-bundle.js";
...
const generated = await generationPage.evaluate(
  async ({ targetSource, candidateSettings, renderWidth, renderHeight }) => {
    ...
    // eslint-disable-next-line no-undef
    const studies = RasterDriftTraceMod.rasterDriftTracePass3StudiesToSvg({
      width: renderWidth,
      height: renderHeight,
      intensity,
    });
    for (const [strategy, svg] of Object.entries(studies)) {
      // eslint-disable-next-line no-undef
      RasterDriftTraceMod.assertRasterDriftTraceSvg(svg, { requireCubic: strategy !== "A3" });
    }
    return {
...
const svgPaths = {
  A3: resolve(outDir, "CF-16-pixel-drift-PASS3-A3-fidelity.svg"),
  B3: resolve(outDir, "CF-16-pixel-drift-PASS3-B3-dense-strands.svg"),
  C3: resolve(outDir, "CF-16-pixel-drift-PASS3-C3-disciplined-hybrid.svg"),
};
const vectorStats = {};
for (const [strategy, path] of Object.entries(svgPaths)) {
  ...
  if (strategy !== "A3" && cubicPathCount === 0) {
    throw new Error(`${path} must include reconstructed cubic strand paths.`);
  }
  vectorStats[strategy] = { path, paths: pathCount, cubicPaths: cubicPathCount, bytes: statSync(path).size };
}
...
const targetPreviewPath = resolve(outDir, "CF-16-pixel-drift-PASS3-TARGET.png");
const rasterDeltaPath = resolve(outDir, "CF-16-pixel-drift-PASS3-RASTER-DELTA.png");
await renderRasterCard("TARGET", "TARGET-twizzler.png · pass-3 evidence basis", targetData, targetPreviewPath);
await renderRasterCard(
  "Δ",
  "original raster pixel-error reference · adaptive contour/ridge source",
  generated.differenceData,
  rasterDeltaPath,
);

const previewPaths = {
  A3: resolve(outDir, "CF-16-pixel-drift-PASS3-A3-fidelity.png"),
  B3: resolve(outDir, "CF-16-pixel-drift-PASS3-B3-dense-strands.png"),
  C3: resolve(outDir, "CF-16-pixel-drift-PASS3-C3-disciplined-hybrid.png"),
};
await renderVectorCard(
  "A3",
  "fidelity trace · adaptive disjoint contour bands preserve white channels",
  generated.studies.A3,
  previewPaths.A3,
);
await renderVectorCard(
  "B3",
  "dense editable reconstruction · direction-continuous cubic crossings",
  generated.studies.B3,
  previewPaths.B3,
);
await renderVectorCard(
  "C3",
  "disciplined hybrid · restrained contour mass + dense cubic texture",
  generated.studies.C3,
  previewPaths.C3,
);

const stackPath = resolve(outDir, "CF-16-pixel-drift-PASS3-COMPARISON-STACK.png");
const stackCards = [targetPreviewPath, rasterDeltaPath, previewPaths.A3, previewPaths.B3, previewPaths.C3]
...
const manifest = {
  ...
  svgs: vectorStats,
  tradeoffs: {
    A3: "Closest shape trace: adaptive disjoint intensity bands preserve channels but remain non-strand contour geometry.",
    B3: "Denser editable cubic ridge reconstruction preserves independent crossing direction but approximates source topology.",
    C3: "Restrained contour support improves mass fidelity while denser cubic ridges carry editable line texture.",
  },
  winner: null,
};
const manifestPath = resolve(outDir, "CF-16-pixel-drift-PASS3-manifest.json");
